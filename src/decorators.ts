function Logger(logString: string) {
  ...
  function Autobind(_: any, _2: string, descriptor: PropertyDescriptor) { //Here 1st and 2nd args are required, but we do not use it in our autobind decorator, so we denoted those arguments as _ and _2 which will tell TS that it will not be used, so the warning will be gone. (We can also write _Toushif - anything after an underscore)
    /* console.log('Method decorator!-----------');
    console.log(target);
    console.log(name);
    console.log(descriptor);
